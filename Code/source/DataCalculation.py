...
class DataCalculation(Calculation):
    ...
    def CalDataReturn(self):
        ...
        CanoeData = {
            0: {"Length": self.Length, "Width": self.Width,
                "Depth": self.Depth, "ECurveF": self.ECurveF, "EWidthF": self.EWidthF,
                "EDepthF": self.EDepthF, "CoverLength": self.CoverLength,
                "Density": self.Density, "CrewWeight": self.CrewWeight, "Thickness": self.Thickness},
    ...
    def Canoe_Flowability(self):
        # Flowability not means: (https://www.merriam-webster.com/dictionary/flowability)
        # the capacity to move by flow that characterizes fluids and loose particulate solids.

        # It means that the ability to flow on water
